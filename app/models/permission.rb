...
class Permission < ActiveRecord::Base
  ...
  PERMISSIONS = {
    ...
    '投标列表' => {action: 'manage', subject: "/backend/bids"},
    '债权转让列表' => {action: 'manage', subject: "/backend/bids/selling"},
    '已转让债权' => {action: 'manage', subject: "/backend/bids/sold"},
    '自投队列' => {action: 'manage', subject: "/backend/auto_invest_rules"},
    '自投统计' => {action: 'manage', subject: "/backend/auto_invest_rules/stats"},
    '还款管理' => {action: 'manage', subject: "/backend/repayments"},
    '系统参数设置' => {action: 'manage', subject: "/backend/system_configs"},
    '邀请、注册奖励设置' => {action: 'manage', subject: "/backend/system_configs/promotion"},
    '投标奖励设置' => {action: 'manage', subject: "/backend/system_configs/prize"},
    '产品推广设置' => {action: 'manage', subject: "/backend/system_configs?type=seo"},
    '线下充值银行设置' => {action: 'manage', subject: "/backend/offline_banks"},
    ...
    '权限列表' => {action: 'manage', subject: "/backend/permissions"},
    ...
    '邀请注册列表' => {action: 'manage', subject: "/backend/invites"},
    ...
    '线下充值审核' => {action: 'manage', subject: "/backend/offline_recharges/wait_audit"},
    '后台充值/扣款' => {action: 'manage', subject: "/backend/pay_orders/manage"},
    '线上充值记录' => {action: 'manage', subject: "/backend/pay_orders"},
    '线下充值记录' => {action: 'manage', subject: "/backend/offline_recharges"},
    '后台充值记录' => {action: 'manage', subject: "/backend/pay_orders/backend_recharges"},
    '后台扣款记录' => {action: 'manage', subject: "/backend/pay_orders/deducts"},
    ...
    '转盘活动' => {action: 'manage', subject: "/backend/promotions/circle"},
    '砸蛋活动' => {action: 'manage', subject: "/backend/promotions/egg"},
    '幸运码' => {action: 'manage', subject: "/backend/promotions/lucky_code"},
    '幸运猜猜猜' => {action: 'manage', subject: "/backend/promotions/lucky_guess"},
    '互动红包' => {action: 'manage', subject: "/backend/promotions/huodong_comment"},
    '运营中心' => {action: 'manage', subject: "/backend/lenders/branch"},
    '余额统计' => {action: 'manage', subject: "/backend/web_statistics/remainder"}
  }
